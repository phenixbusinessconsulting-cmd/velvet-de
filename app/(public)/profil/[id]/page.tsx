...
import { MapPin, Globe, Clock, CheckCircle2, Star, ChevronLeft, Car, Home } from "lucide-react"
import { Header } from "@/components/layout/header"
import { Footer } from "@/components/layout/footer"
import { PhotoLightbox, type MediaItem } from "@/components/features/public/photo-lightbox"
import { PrivateGalleryLock } from "@/components/features/public/private-gallery-lock"
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const profile = await prisma.professionalProfile.findFirst({
    where: { slug: id, status: "APPROVED" },
    include: { city: true },
  })
  if (!profile) return { title: "Profil nicht gefunden / Profil introuvable" }

  return {
    title: `${profile.displayName}, ${profile.age} — ${profile.city.nameDE}`,
    description: profile.metaDescription ?? profile.tagline ?? undefined,
  }
}
...
export default async function ProfilePage({ params }: Props) {
  ...
  const profile = await prisma.professionalProfile.findFirst({
    where: { slug: id, status: "APPROVED", kycStatus: "APPROVED" },
    include: {
      city: true,
      photos: {
      ...
      _count: {
        select: {
          ...
          photos: { where: { isApproved: true, isPrivate: true } },
          videos: { where: { isApproved: true, isPrivate: true } },
        },
      },
    },
  })
  ...
  // When banner is shown as hero, all photos go to gallery; otherwise exclude the main portrait
  const galleryPhotos = profile.bannerUrl
    ? profile.photos
    : profile.photos.filter((ph) => !ph.isMain)

  const firstPhotoUrl = profile.photos[0]?.cdnUrl
  const privatePhotoCount = profile._count.photos
  const privateVideoCount = profile._count.videos
  const hasPrivateContent = privatePhotoCount > 0 || privateVideoCount > 0
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-2 space-y-4">
              {/* Main photo — banner (16:9) if set, else portrait */}
              ...
              {hasPrivateContent && (
                <div className="card-luxury p-5">
                  <PrivateGalleryLock
                    ...
                    price={profile.privateGalleryPrice ? Number(profile.privateGalleryPrice) : null}
                    privatePhotoCount={privatePhotoCount}
                    privateVideoCount={privateVideoCount}
                    placeholderUrl={firstPhotoUrl ?? null}
                  />
                </div>
              )}
              ...
              {profile.bio && (
                <div className="card-luxury p-7">
                  <h2
                    className="text-xl font-light text-[var(--pearl)] mb-4"
              ...
              {profile.reviews.length > 0 && (
                <div className="card-luxury p-7">
                  <div className="flex items-center justify-between mb-6">
                    <h2
                      className="text-xl font-light text-[var(--pearl)]"
                      style={{ fontFamily: "var(--font-display)" }}
                    >
                      {p.reviews}
                    </h2>
            ...
            <div className="space-y-5">
              ...
              <div className="card-luxury p-6">
                <h1
                  className="text-3xl font-light text-[var(--pearl)] mb-1"
                  style={{ fontFamily: "var(--font-display)" }}
                >
                  {profile.displayName}
              ...
              >
                ...
                <Button
                  ...
                  className="w-full"
                >
                  <Link href={`/kontakt?profile=${profile.slug}`}>
                    {p.contactButton}
                  </Link>
                </Button>
              ...
              {profile.availability.length > 0 && (
                <div className="card-luxury p-5">
                  <h3
                    className="text-base font-light text-[var(--pearl)] mb-4 flex items-center gap-2"
                    style={{ fontFamily: "var(--font-display)" }}
                  >
              ...
              {profile.servicesTags.length > 0 && (
                <div className="card-luxury p-5">
                  <h3
                    className="text-base font-light text-[var(--pearl)] mb-4"
                    style={{ fontFamily: "var(--font-display)" }}
                  >
                    {p.characteristics}
                  </h3>
              ...
              <div className="text-center">
                <Link
                  href={`/kontakt?type=report&profile=${profile.slug}`}
