...
export const metadata: Metadata = {
  ...
  description: "Impressum der VELVET Group GmbH gemäß §5 TMG.",
}

export default async function ImpressumPage() {
  const [page, locale] = await Promise.all([
    prisma.legalPage.findUnique({ where: { slug: "impressum" } }),
    getLocale(),
  ])

  const isFr   = locale === "fr"
  const title   = isFr ? (page?.titleFR   || "Mentions légales")        : (page?.titleDE   || "Impressum")
